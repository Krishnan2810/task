...
//pricing table
if($('.dt-sc-pricing-block').length > 0 ){
var allItems = document.querySelectorAll(".dt-sc-pricing-block");
 var items = document.querySelector(".dt-sc-pricing-blocks-section").querySelectorAll(".dt-sc-pricing-block");
  items.forEach(item => {    
...
function numberCounter() {       
	var dataId = $('.whistle-home-custom').attr('data-section-id');  
...
document.querySelectorAll('[id^="shopify-section-"]').forEach((card, index) => {  
   if(card.classList.contains('dt-support-section__primary')) {        
    const topHeading = card.querySelectorAll('.support-block-card__info');        
    const cardHeight = parseInt(topHeading[0].clientHeight);
    const topValue = cardHeight / 2 + 5;
    const cardTopCSS = "calc(50% - "+topValue+'px)';     
    const swiperContainer = card.querySelector('.swiper');    
